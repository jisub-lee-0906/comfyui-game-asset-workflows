...
RUN_DIR = PROJECT_ROOT / 'docs/automation/generation_runs/unit7h_screen_device_prompt_research'
CANDIDATE_DIR = PROJECT_ROOT / 'docs/automation/generated_candidates/props/unit7h_screen_device_prompt_research'
...
NEG = (
    ...
    'status bar, notification, symbol, cup, glass, drink, keyboard, mouse, cable'
)

VARIANTS = [
    {
        'id': 'A_face_down_back_side',
        'positive': (
            'smartphone face down, back side of phone, plain phone back, single smartphone, desk, shadow, '
            'still_life, object_focus, no_humans, no visible screen, no logo, no camera mark, no text, '
    ...
    },
    {
        'id': 'B_side_edge_view',
        'positive': (
            'smartphone side view, thin black phone edge, screen not visible, single smartphone, desk, shadow, '
    ...
    {
        'id': 'C_featureless_black_glass',
        'positive': (
            'featureless black glass smartphone, plain black rectangle phone, unlit screen, pure black glass, '
            'smartphone, desk, shadow, still_life, object_focus, no_humans, no notch, no home button, '
            'no logo, no icons, no text, dark reflection only, depth_of_field, masterpiece, best_quality, very_aesthetic, high_resolution'
        ),
    },
    {
        'id': 'D_strict_front_blank_screen',
        'positive': (
            'blank black smartphone screen, screen turned off, plain smartphone, smartphone, screen, glass, desk, shadow, '
            'still_life, object_focus, no_humans, empty display, no logo, no icons, no app UI, no status bar, '
            'no home button, no text, depth_of_field, masterpiece, best_quality, very_aesthetic, high_resolution'
        ),
    },
]
...
def main() -> None:
    ...
    for i, variant in enumerate(VARIANTS, 1):
        ...
        run_id = f"unit7h_{i:02d}_{variant['id']}"
        ...
        results.append({'variant': variant['id'], 'prompt_id': prompt_id, 'seed': SEED, 'positive': variant['positive'], 'negative': NEG, 'copies': copies})
    (RUN_DIR / 'unit7h_screen_device_prompt_research.json').write_text(json.dumps(results, ensure_ascii=False, indent=2), encoding='utf-8')
