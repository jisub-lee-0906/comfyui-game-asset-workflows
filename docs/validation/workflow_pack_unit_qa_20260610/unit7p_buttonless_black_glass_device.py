...
RUN_DIR = PROJECT_ROOT / 'docs/automation/generation_runs/unit7p_buttonless_black_glass_device'
CANDIDATE_DIR = PROJECT_ROOT / 'docs/automation/generated_candidates/props/unit7p_buttonless_black_glass_device'
...
NEG = (
    ...
    'status bar, notification, symbol, corner mark, small mark, colored dot, green icon, white icon, '
    'indicator light, notification light, home button, physical button, button ring, notch, speaker slot, '
    'camera hole, app screen, active screen, monitor, television, computer monitor, large display, background screen, '
    'cup, glass, drink, keyboard, mouse, cable, iphone'
)

PROMPT = (
    'buttonless black glass slab shaped like a smartphone, minimal touchscreen device prop, '
    'featureless empty black glass surface, display powered off, completely blank dark reflective glass, '
    'single device only on wooden tabletop, wooden_table, shadow, still_life, object_focus, no_humans, '
    'tabletop fills background, no logo, no icons, no app UI, no text, no corner marks, no colored marks, '
    'no home button, no notch, no speaker slot, no monitor, no large display, simple modern item cut-in, '
    'depth_of_field, masterpiece, best_quality, very_aesthetic, high_resolution'
)

SEEDS = [360610727, 360610728, 360610729]
...
def main() -> None:
    ...
    for i, seed in enumerate(SEEDS, 1):
        ...
        run_id = f"unit7p_{i:02d}_buttonless_black_glass_seed_{seed}"
        ...
        results.append({'variant': run_id, 'prompt_id': prompt_id, 'seed': seed, 'positive': PROMPT, 'negative': NEG, 'copies': copies})
    (RUN_DIR / 'unit7p_buttonless_black_glass_device.json').write_text(json.dumps(results, ensure_ascii=False, indent=2), encoding='utf-8')
