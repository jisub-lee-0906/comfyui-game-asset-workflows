...
RUN_DIR = PROJECT_ROOT / 'docs/automation/generation_runs/unit7n_smartphone_featureless_black_glass'
CANDIDATE_DIR = PROJECT_ROOT / 'docs/automation/generated_candidates/props/unit7n_smartphone_featureless_black_glass'
...
NEG = (
    ...
    'indicator light, notification light, home button, app screen, active screen, monitor, television, '
    'computer monitor, large display, background screen, cup, glass, drink, keyboard, mouse, cable'
)

PROMPT = (
    'small plain smartphone on wooden tabletop, featureless empty black glass front, display powered off, '
    'completely blank dark reflective glass surface, single smartphone only, wooden_table, shadow, still_life, '
    'object_focus, no_humans, tabletop fills background, no logo, no icons, no app UI, no text, no corner marks, '
    'no colored marks, no monitor, no large display, simple modern item cut-in, depth_of_field, masterpiece, '
    'best_quality, very_aesthetic, high_resolution'
)

SEEDS = [360610723, 360610724, 360610725]
...
def main() -> None:
    ...
    for i, seed in enumerate(SEEDS, 1):
        ...
        run_id = f"unit7n_{i:02d}_featureless_black_glass_seed_{seed}"
    ...
    (RUN_DIR / 'unit7n_smartphone_featureless_black_glass.json').write_text(json.dumps(results, ensure_ascii=False, indent=2), encoding='utf-8')
