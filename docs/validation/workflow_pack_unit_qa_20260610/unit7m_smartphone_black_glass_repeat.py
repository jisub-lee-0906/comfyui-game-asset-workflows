...
RUN_DIR = PROJECT_ROOT / 'docs/automation/generation_runs/unit7m_smartphone_black_glass_repeat'
CANDIDATE_DIR = PROJECT_ROOT / 'docs/automation/generated_candidates/props/unit7m_smartphone_black_glass_repeat'
...
NEG = (
    ...
    'status bar, notification, symbol, cup, glass, drink, keyboard, mouse, cable, monitor, television, '
    'computer monitor, large display, background screen, active screen, app screen'
)

PROMPT = (
    'small plain smartphone on wooden tabletop, black glass front, phone display off, dark reflective glass, '
    'single smartphone only, wooden_table, shadow, still_life, object_focus, no_humans, tabletop fills background, '
    'no logo, no icons, no app UI, no text, no monitor, no large display, simple modern item cut-in, '
    'depth_of_field, masterpiece, best_quality, very_aesthetic, high_resolution'
)

SEEDS = [360610719, 360610720, 360610721]
...
def main() -> None:
    ...
    for i, seed in enumerate(SEEDS, 1):
        ...
        run_id = f"unit7m_{i:02d}_smartphone_black_glass_seed_{seed}"
    ...
    (RUN_DIR / 'unit7m_smartphone_black_glass_repeat.json').write_text(json.dumps(results, ensure_ascii=False, indent=2), encoding='utf-8')
