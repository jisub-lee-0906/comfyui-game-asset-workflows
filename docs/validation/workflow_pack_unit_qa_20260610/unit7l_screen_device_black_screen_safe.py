...
RUN_DIR = PROJECT_ROOT / 'docs/automation/generation_runs/unit7l_screen_device_black_screen_safe'
CANDIDATE_DIR = PROJECT_ROOT / 'docs/automation/generated_candidates/props/unit7l_screen_device_black_screen_safe'
...
NEG = (
    ...
    'status bar, notification, symbol, cup, glass, drink, keyboard, mouse, cable, monitor, active screen, app screen'
)

VARIANTS = [
    {
        'id': 'A_smartphone_black_screen_safe',
        'seed': 360610717,
        'positive': (
            'plain smartphone, blank black screen, screen turned off, dark reflective screen, smartphone, phone, desk, shadow, '
            'still_life, object_focus, no_humans, no logo, no icons, no app UI, no text, simple modern item cut-in, '
            'depth_of_field, masterpiece, best_quality, very_aesthetic, high_resolution'
        ),
    },
    {
        'id': 'B_tablet_pc_black_screen_safe',
        'seed': 360610718,
        'positive': (
            'plain tablet_pc, blank black screen, screen turned off, dark reflective screen, tablet device, desk, shadow, '
            'still_life, object_focus, no_humans, no logo, no icons, no app UI, no text, simple modern item cut-in, '
            'depth_of_field, masterpiece, best_quality, very_aesthetic, high_resolution'
        ),
    },
...
def main() -> None:
    ...
    for i, variant in enumerate(VARIANTS, 1):
        ...
        run_id = f"unit7l_{i:02d}_{variant['id']}"
    ...
    (RUN_DIR / 'unit7l_screen_device_black_screen_safe.json').write_text(json.dumps(results, ensure_ascii=False, indent=2), encoding='utf-8')
